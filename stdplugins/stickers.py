...
Available Commands:
.kang [Optional Emoji] // steal stickers
.pkang [Optional Emoji] // Make Sticker from Picture
.packinfo
.getsticker"""
...
async def _(event):
    ...
    packname = f"Stolen By : {userid}"
    packshortname = f"sticker_{userid}" 

    is_a_s = is_it_animated_sticker(reply_message)
    file_ext_ns_ion = "Lawda.png"
    file = await borg.download_file(reply_message.media)
    uploaded_sticker = None
    if is_a_s:
        file_ext_ns_ion = "AnimatedSticker.tgs"
        uploaded_sticker = await borg.upload_file(file, file_name=file_ext_ns_ion)
        packname = f"Stolen Anim.. {userid}"
        packshortname = f"Anim_{userid}"  # format: Uni_Borg_userid
